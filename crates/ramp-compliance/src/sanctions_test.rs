...
async fn test_sanctions_integration() {
    ...
    let result = engine
        .check_transaction(&clean_tx)
        .await
        .expect("Failed to check clean transaction");
    ...
    let result = engine
        .check_transaction(&sanctioned_tx)
        .await
        .expect("Failed to check sanctioned transaction");
    assert!(!result.passed);
    assert!(result.requires_review);
    ...
    let result = engine
        .check_transaction(&address_tx)
        .await
        .expect("Failed to check address transaction");
    assert!(result.passed);
}
