...
mod tests {
    use super::*;
    use crate::types::{CaseSeverity, CaseType};

    fn create_test_rule() -> RuleDefinition {
        RuleDefinition {
            id: "test-rule-1".to_string(),
            name: "Test Rule".to_string(),
            description: Some("A test rule".to_string()),
            rule_type: Some("velocity".to_string()),
            severity: crate::rule_parser::SeverityLevel::Medium,
            enabled: true,
    ...
    #[test]
    fn test_rule_version_serialization() {
        let version = RuleVersion {
            id: Uuid::now_v7(),
            tenant_id: TenantId::new("tenant1"),
            version_number: 1,
            rules_json: serde_json::to_value(vec![create_test_rule()]).unwrap(),
            is_active: true,
            created_at: Utc::now(),
        ...
        assert_eq!(version.version_number, parsed.version_number);
        assert_eq!(version.is_active, parsed.is_active);
    }
}
*/
