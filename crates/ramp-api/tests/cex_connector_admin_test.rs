...
const TEST_API_KEY: &str = "cex_connector_test_api_key";
const TEST_API_SECRET: &str = "cex_connector_test_api_secret";
const TEST_ADMIN_KEY: &str = "cex_connector_admin_key";
...
async fn setup_app_with_pool(tenant_id: &str, db_pool: Option<PgPool>) -> TestApp {
    ...
    tenant_repo.add_tenant(TenantRow {
        ...
        name: "CEX Connector Test Tenant".to_string(),
    ...
    let app_state = AppState {
        ...
        portal_auth_config: Arc::new(PortalAuthConfig {
            jwt_secret: "cex-connector-test-secret".to_string(),
            issuer: None,
            audience: None,
...
async fn seed_db_tenant(pool: &PgPool, tenant_id: &str, api_key_hash: &str) {
    ...
    .bind(tenant_id)
    .bind("CEX Connector Test Tenant")
    .bind(api_key_hash)
    .execute(pool)
    .await
...
async fn seed_cex_connector_graph(pool: &PgPool, tenant_id: &str) {
    let service = VenueTrustService::with_pool(pool.clone());

    service
        .upsert_connection(&UpsertVenueConnectionRequest {
            connection_id: "conn-binance-1".to_string(),
            tenant_id: tenant_id.to_string(),
            subject_type: "user".to_string(),
            subject_id: "user-cex-1".to_string(),
            user_id: Some("user-cex-1".to_string()),
    ...
    service
        .upsert_account(&UpsertVenueAccountRequest {
            account_id: "acct-binance-1".to_string(),
            tenant_id: tenant_id.to_string(),
            venue_connection_id: "conn-binance-1".to_string(),
            venue_key: "binance".to_string(),
...
#[tokio::test]
async fn cex_connector_readiness_snapshot_returns_generic_operator_readiness_view() {
    unsafe {
    ...
    seed_db_tenant(&pool, "tenant_cex_connector", &api_key_hash).await;
    seed_cex_connector_graph(&pool, "tenant_cex_connector").await;

    let app = setup_app_with_pool("tenant_cex_connector", Some(pool)).await;
    let request = build_signed_admin_request(
        "GET",
        "/v1/admin/venue-trust/connectors/cex/binance/readiness/user/user-cex-1",
        "",
        &app.api_key,
        &app.api_secret,
        TEST_ADMIN_KEY,
    ...
    assert_eq!(payload["connectorKey"], "binance");
    assert_eq!(payload["status"], "ready");
    assert_eq!(payload["connectionId"], "conn-binance-1");
    assert_eq!(payload["accountId"], "acct-binance-1");
    assert_eq!(payload["apiKeyMode"], "scoped_read_write");
    assert_eq!(payload["subaccountMode"], "segregated");
...
#[tokio::test]
async fn cex_connector_readiness_snapshot_fails_closed_without_runtime() {
    unsafe {
        std::env::set_var("RAMPOS_ADMIN_KEY", TEST_ADMIN_KEY);
    }

    let app = setup_app_with_pool("tenant_cex_connector_no_runtime", None).await;
    let request = build_signed_admin_request(
        "GET",
        "/v1/admin/venue-trust/connectors/cex/binance/readiness/user/user-cex-1",
        "",
    ...
    assert!(
        ...
        "no-runtime cex readiness should return structured error payload"
    );
}

#[test]
fn openapi_documents_cex_connector_readiness_snapshot_contract() {
    ...
    assert!(
        path.is_object(),
        "spec must document GET /v1/admin/venue-trust/connectors/cex/{{connector_key}}/readiness/{{subject_type}}/{{subject_id}}"
    );

    let response_schema = path["responses"]["200"]["content"]["application/json"]["schema"]
        .as_object()
        .expect("cex readiness response schema must exist");
    let required = response_schema["required"]
        .as_array()
        .expect("cex readiness response schema must define required fields")
        .iter()
    ...
    for field in [
        ...
        assert!(
            ...
            "cex readiness response must require '{}'",
            field
        );
    }
}
