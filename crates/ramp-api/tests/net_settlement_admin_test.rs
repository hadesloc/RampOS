...
const TEST_API_KEY: &str = "net_settlement_test_api_key";
const TEST_API_SECRET: &str = "net_settlement_test_api_secret";
const TEST_ADMIN_JWT_SECRET: &str = "net-settlement-admin-jwt-secret";
...
fn make_admin_jwt(role: &str) -> String {
    let claims = ramp_api::handlers::admin::admin_auth::AdminClaims {
        sub: "net_settlement_admin_test_user".to_string(),
        email: "net-settlement-admin@rampos.local".to_string(),
        role: role.to_string(),
...
async fn setup_app(tenant_id: &str) -> TestApp {
    ...
    tenant_repo.add_tenant(TenantRow {
        ...
        name: "Net Settlement Test Tenant".to_string(),
    ...
    let app_state = AppState {
        ...
        portal_auth_config: Arc::new(PortalAuthConfig {
            jwt_secret: "net-settlement-test-secret".to_string(),
            issuer: None,
            audience: None,
...
#[tokio::test]
async fn settlement_workbench_returns_approval_gated_snapshot() {
    std::env::set_var("RAMPOS_ADMIN_JWT_SECRET", TEST_ADMIN_JWT_SECRET);
    let app = setup_app("tenant_net_settlement_workbench").await;
    ...
    assert_eq!(payload["actionMode"], "approval_gated");
    assert_eq!(payload["approvalMode"], "manual_approval");
    assert!(payload["snapshot"]["proposals"]
        .as_array()
        .unwrap()
        .iter()
    ...
    assert!(payload["snapshot"]["proposals"][0]["approvalReferenceId"]
        .as_str()
        .unwrap()
...
#[tokio::test]
async fn settlement_workbench_export_returns_csv_attachment() {
    std::env::set_var("RAMPOS_ADMIN_JWT_SECRET", TEST_ADMIN_JWT_SECRET);
    let app = setup_app("tenant_net_settlement_export").await;
    let admin_jwt = make_admin_jwt("viewer");

    let request = build_signed_admin_jwt_request(
        "GET",
        "/v1/admin/settlement/export?format=csv",
        "",
        &app.api_key,
        &app.api_secret,
        &admin_jwt,
