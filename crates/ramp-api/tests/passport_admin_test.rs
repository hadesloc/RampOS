...
async fn setup_app(tenant_id: &str) -> TestApp {
    ...
    tenant_repo.add_tenant(TenantRow {
        ...
        name: "Passport Test Tenant".to_string(),
    ...
    let app_state = AppState {
        ...
        portal_auth_config: Arc::new(PortalAuthConfig {
            jwt_secret: "passport-test-secret".to_string(),
            issuer: None,
            audience: None,
...
async fn passport_queue_lists_items() {
    ...
    assert_eq!(payload["actionMode"], "consent_review");
...
async fn passport_package_returns_detail() {
    ...
    let body = to_bytes(response.into_body(), usize::MAX).await.unwrap();
    let payload: serde_json::Value = serde_json::from_slice(&body).unwrap();
    assert_eq!(payload["packageId"], "pkg_passport_active_001");
    assert_eq!(payload["consentStatus"], "granted");
}
