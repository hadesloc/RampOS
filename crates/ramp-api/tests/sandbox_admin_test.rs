...
async fn setup_app() -> TestApp {
    ...
    tenant_repo.add_tenant(TenantRow {
        id: "tenant_sandbox_test".to_string(),
        name: "Sandbox Test Tenant".to_string(),
    ...
    let app_state = AppState {
        ...
        portal_auth_config: Arc::new(PortalAuthConfig {
            jwt_secret: "sandbox-test-secret".to_string(),
            issuer: None,
            audience: None,
...
async fn sandbox_admin_lists_default_presets() {
    ...
    assert!(payload.as_array().unwrap().len() >= 3);
    assert_eq!(payload[0]["code"], "BASELINE");
}
...
async fn sandbox_admin_seeds_tenant_from_default_preset_catalog() {
    ...
    assert_eq!(payload["tenantName"], "Seeded Sandbox Tenant");
    assert_eq!(payload["presetCode"], "BASELINE");
    assert_eq!(payload["scenarioCode"], "PAYIN_BASELINE");
}
