...
async fn setup_app(tenant_id: &str) -> TestApp {
    ...
    tenant_repo.add_tenant(TenantRow {
        ...
        name: "Treasury Test Tenant".to_string(),
    ...
    let app_state = AppState {
        ...
        portal_auth_config: Arc::new(PortalAuthConfig {
            jwt_secret: "treasury-test-secret".to_string(),
            issuer: None,
            audience: None,
...
async fn treasury_workbench_supports_stable_fixture_scenario() {
    ...
    assert!(payload["snapshot"]["forecasts"]
        .as_array()
        .unwrap()
        .iter()
        .all(|item| item["shortageAmount"] == "0"));
}
