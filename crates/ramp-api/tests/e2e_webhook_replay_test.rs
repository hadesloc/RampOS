...
fn generate_signature(
    method: &str, path: &str, timestamp: &str, body: &str, secret: &str,
) -> String {
...
fn build_admin_request(
    ...
    api_key: &str, api_secret: &str, admin_key: &str,
) -> Request<Body> {
...
async fn setup_app() -> TestApp {
    ...
    let onboarding_service = Arc::new(ramp_core::service::onboarding::OnboardingService::new(
        tenant_repo.clone(), ledger_service.clone(),
    ));
    ...
    let report_generator = Arc::new(ReportGenerator::new(
        pool, Arc::new(MockDocumentStorage::new()),
    ));
    ...
    let app_state = AppState {
        ...
        webhook_service: Arc::new(
            ramp_core::service::webhook::WebhookService::new(
                webhook_repo.clone(), tenant_repo.clone(),
            ).unwrap(),
        ),
        tenant_repo,
        intent_repo,
...
async fn viewer_cannot_replay_events() {
    ...
    app.webhook_repo
        .queue_event(&sample_event("evt_no_replay_viewer", "intent.status.changed"))
        .await
        .unwrap();
...
async fn viewer_cannot_retry_events() {
    ...
    app.webhook_repo
        .queue_event(&sample_event("evt_no_retry_viewer", "intent.status.changed"))
        .await
        .unwrap();
