...
const TEST_API_KEY: &str = "rescreening_test_api_key";
const TEST_API_SECRET: &str = "rescreening_test_api_secret";
const TEST_ADMIN_JWT_SECRET: &str = "rescreening-admin-jwt-secret";
...
fn make_admin_jwt(role: &str) -> String {
    let claims = ramp_api::handlers::admin::admin_auth::AdminClaims {
        sub: "rescreening_admin_test_user".to_string(),
        email: "rescreening-admin@rampos.local".to_string(),
        role: role.to_string(),
...
async fn setup_app(tenant_id: &str) -> TestApp {
    ...
    tenant_repo.add_tenant(TenantRow {
        ...
        name: "Rescreening Test Tenant".to_string(),
    ...
    user_repo.add_user(UserRow {
        id: "user_rescreen_due".to_string(),
        tenant_id: tenant_id.to_string(),
        status: "ACTIVE".to_string(),
        kyc_tier: 2,
        kyc_status: "VERIFIED".to_string(),
        kyc_verified_at: Some(Utc::now() - Duration::days(240)),
        risk_score: None,
        risk_flags: serde_json::json!({
            "rescreening": {
                "nextRunAt": (Utc::now() - Duration::days(1)).to_rfc3339(),
                "documentExpiryAt": (Utc::now() + Duration::days(14)).to_rfc3339(),
    ...
    let app_state = AppState {
        ...
        portal_auth_config: Arc::new(PortalAuthConfig {
            jwt_secret: "rescreening-test-secret".to_string(),
            issuer: None,
            audience: None,
...
async fn rescreening_runs_lists_due_users() {
    ...
    assert_eq!(payload[0]["userId"], "user_rescreen_due");
    assert_eq!(payload[0]["triggerKind"], "document_expiry");
    assert_eq!(payload[0]["restrictionStatus"], "NONE");
}
...
async fn rescreening_restriction_action_updates_user_flags() {
    ...
    let request = build_signed_admin_jwt_request(
        "POST",
        "/v1/admin/rescreening/users/user_rescreen_due/restrict",
        &body,
        &app.api_key,
        &app.api_secret,
        &operator_jwt,
