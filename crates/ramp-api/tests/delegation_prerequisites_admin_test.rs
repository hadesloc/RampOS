...
const TEST_API_KEY: &str = "delegation_prerequisites_test_api_key";
const TEST_API_SECRET: &str = "delegation_prerequisites_test_api_secret";
const TEST_ADMIN_KEY: &str = "delegation_prerequisites_admin_key";
...
async fn setup_app(tenant_id: &str) -> TestApp {
    ...
    tenant_repo.add_tenant(TenantRow {
        ...
        name: "Delegation Prerequisites Test Tenant".to_string(),
    ...
    let report_pool = sqlx::PgPool::connect_lazy("postgres://postgres:postgres@localhost/postgres")
    ...
    let app_state = AppState {
        ...
        portal_auth_config: Arc::new(PortalAuthConfig {
            jwt_secret: "delegation-prerequisites-test-secret".to_string(),
            issuer: None,
            audience: None,
...
#[tokio::test]
async fn delegation_prerequisites_snapshot_returns_structured_read_only_admin_view() {
    unsafe {
        std::env::set_var("RAMPOS_ADMIN_KEY", TEST_ADMIN_KEY);
    }

    let app = setup_app("tenant_delegation_prerequisites").await;
    let request = build_signed_admin_request(
        "GET",
        "/v1/admin/venue-trust/delegation-prerequisites/user/user-delegation-1/delegates/0x00000000000000000000000000000000000000aa?tool_surface=portal_admin&approval_boundary=operator_explicit",
        "",
        &app.api_key,
        &app.api_secret,
        TEST_ADMIN_KEY,
    ...
    assert_eq!(payload["subjectId"], "user-delegation-1");
    assert_eq!(
        payload["delegate"],
        "0x00000000000000000000000000000000000000aa"
    );
    assert_eq!(
        payload["prerequisites"]["allowedToolSurfaces"][0],
        "portal_admin"
    );
    assert_eq!(
        payload["prerequisites"]["approvalBoundary"],
        "operator_explicit"
    );
