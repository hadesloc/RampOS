...
impl PaymasterService {
    pub fn new(paymaster_address: Address, signer_key: Vec<u8>) -> Result<Self> {
        ...
        let key_bytes: [u8; 32] = signer_key.try_into().map_err(|_| {
            ramp_common::Error::Validation(
                "Signer key must be exactly 32 bytes for secp256k1".into(),
            )
        })?;

        let signing_key = SigningKey::from_bytes(&key_bytes.into()).map_err(|e| {
            ramp_common::Error::Validation(format!("Invalid secp256k1 private key: {}", e))
        })?;
