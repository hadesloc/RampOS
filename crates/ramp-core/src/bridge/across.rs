...
impl CrossChainBridge for AcrossBridge {
    ...
    async fn bridge(&self, quote: BridgeQuote) -> Result<TxHash> {
        ...
        // transaction submission and confirmation. The execution_data.depositTx
        // field contains everything needed to build the real transaction.
        tracing::info!(
            "Rejecting Across bridge execution for {} -> {} (amount: {}) because transaction submission is not implemented",
            quote.from_chain,
            quote.to_chain,
            quote.amount,
        );
        ...
        Err(Error::NotImplemented(
            "Across bridge execution is experimental and disabled until real transaction submission and confirmation are implemented"
                .to_string(),
        ))
    }
    ...
    async fn test_bridge_execution_fails_closed_without_tx_hash() {
        ...
        let quote = BridgeQuote {
            ...
            execution_data: serde_json::json!({
                "depositTx": { "method": "depositV3" }
            }),
        };

        let err = bridge.bridge(quote).await.unwrap_err();
        assert!(
            matches!(err, Error::NotImplemented(message) if message == "Across bridge execution is experimental and disabled until real transaction submission and confirmation are implemented")
        );
    }
}
