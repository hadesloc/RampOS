...
export default function YieldPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (strategiesData.status === "fulfilled") {
        ...
        // Fallback mock
        setStrategies([
          { id: "aave-usdc", name: "Aave V3 USDC", description: "Lending", riskLevel: "low", maxProtocolExposure: 40, maxTokenExposure: 50, minApyThreshold: 2.0, rebalanceApyThreshold: 1.0, minHealthFactor: 2.0, rebalanceIntervalSecs: 86400, gasAwareRebalancing: true, allowedProtocols: ["aave-v3"], isActive: false },
          { id: "compound-eth", name: "Compound V3 ETH", description: "Lending", riskLevel: "low", maxProtocolExposure: 50, maxTokenExposure: 60, minApyThreshold: 1.0, rebalanceApyThreshold: 0.5, minHealthFactor: 1.5, rebalanceIntervalSecs: 3600, gasAwareRebalancing: true, allowedProtocols: ["compound-v3"], isActive: false },
          { id: "curve-3pool", name: "Curve 3Pool", description: "Liquidity", riskLevel: "medium", maxProtocolExposure: 50, maxTokenExposure: 60, minApyThreshold: 1.0, rebalanceApyThreshold: 0.5, minHealthFactor: 1.5, rebalanceIntervalSecs: 3600, gasAwareRebalancing: true, allowedProtocols: ["curve"], isActive: false },
          { id: "yearn-usdt", name: "Yearn USDT", description: "Vault", riskLevel: "medium", maxProtocolExposure: 70, maxTokenExposure: 80, minApyThreshold: 0.5, rebalanceApyThreshold: 0.3, minHealthFactor: 1.2, rebalanceIntervalSecs: 1800, gasAwareRebalancing: false, allowedProtocols: ["yearn"], isActive: false },
        ]);
      }
  ...
  const totalDeposited = performance ? `$${(parseFloat(performance.totalDeposited) / 1e6).toLocaleString(undefined, { minimumFractionDigits: 2 })}` : "$12,450.00";
  const totalEarned = performance ? `+$${(parseFloat(performance.totalYieldEarned) / 1e6).toLocaleString(undefined, { minimumFractionDigits: 2 })}` : "+$340.50";
  const avgApy = performance ? `${performance.averageApy.toFixed(1)}%` : "6.8%";
            ...
            <CardContent>
              {performance && performance.positions.length > 0 ? (
                ...
                <Table>
                  ...
                  <TableBody>
                    <TableRow>
                      <TableCell className="font-medium">Aave V3</TableCell>
                      <TableCell>USDC</TableCell>
                      <TableCell>$5,000.00</TableCell>
                      <TableCell>$5,125.40</TableCell>
                      <TableCell className="text-green-500">5.2%</TableCell>
                      <TableCell>+$125.40</TableCell>
                      <TableCell className="text-right space-x-2">
                    ...
                    <TableRow>
                      <TableCell className="font-medium">Compound V3</TableCell>
                      <TableCell>ETH</TableCell>
                      <TableCell>2.5 ETH</TableCell>
                      <TableCell>2.55 ETH</TableCell>
