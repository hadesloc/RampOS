    ...
    while (this.stack.length > 0) {
      ...
      while (state.index < state.selectionSet.length) {
        ...
        if (!shouldInclude(select, this.ctx.variables)) ; else if (select.kind !== graphql_web.Kind.FIELD) {
          ...
          if (fragment) {
            var isMatching = !fragment.typeCondition || (this.ctx.store.schema ? isInterfaceOfType(this.ctx.store.schema, fragment, this.typename) : this.ctx.store.possibleTypeMap ? isSuperType(this.ctx.store.possibleTypeMap, fragment.typeCondition.name.value, this.typename) : currentOperation === 'read' && isFragmentMatching(fragment.typeCondition.name.value, this.typename) || isFragmentHeuristicallyMatching(fragment, this.typename, this.entityKey, this.ctx.variables, this.ctx.store.logger));
...
var isSuperType = (possibleTypeMap, typeCondition, typename) => {
  if (!typename) return false;
  if (typeCondition === typename) return true;
  var concreteTypes = possibleTypeMap.get(typeCondition);
...
class Store {
  constructor(opts) {
    ...
    if (!this.schema && opts.possibleTypes) {
      ...
      for (var entry of Object.entries(opts.possibleTypes)) {
        var [abstractType, concreteTypes] = entry;
...
var cacheExchange = opts => ({
  ...
  return operations$ => {
    ...
    var cacheMissOps$ = wonka.map(res => {
      ...
      return addMetadata(res.operation, {
        cacheOutcome: 'miss'
      });
