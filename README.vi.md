...
### 🔧 Lõi xử lý (`ramp-core`)
- **State Machine** — Luồng giao dịch chuẩn hóa cho các intent Nạp tiền, Rút tiền và Giao dịch
- **Intent Solver** — Xử lý intent thống nhất với các backend thực thi có thể thay thế
- **Sổ cái kép** — Giao dịch nguyên tử, cân bằng với độ chính xác `rust_decimal`
- **Webhook Engine** — Giao hàng với retry exponential backoff và ký HMAC-SHA256
- **Idempotency** — Retry an toàn qua theo dõi idempotency key
- **Multi-Tenant** — Phân tách dữ liệu hoàn toàn, cấu hình per-tenant, quản lý API key
- **Job Scheduler** — Background jobs cho xử lý timeout, retry webhook, cảnh báo compliance
...
### ⛓️ Hợp đồng thông minh (Solidity 0.8.24 / Foundry)
...
```
┌──────────────────┐     ┌──────────────────┐     ┌──────────────────┐
│   Sàn giao dịch  │     │     RampOS        │     │  Ngân hàng / PSP │
